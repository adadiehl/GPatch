...
THREADS=24
ASM_MODE="asm20"  # Available values are asm5, asm10, asm20
...
minimap2 -x $ASM_MODE -t $THREADS -a $REFERENCE_FASTA $ASSEMBLY_FASTA | samtools view -b - > $PREFIX.bam 2> $PREFIX.GPatch.err
...
minimap2 -x $ASM_MODE -t $THREADS $REFERENCE_FASTA $PREFIX.patched.fasta > $PREFIX.patched.$REFERENCE_NAME.paf 2>> $PREFIX.GPatch.err
...
minimap2 -x $ASM_MODE -t $THREADS -a $REFERENCE_FASTA $PREFIX.cbreak.fa | samtools view -b - > $PREFIX.cbreak.bam 2>> $PREFIX.GPatch.err
...
minimap2 -x $ASM_MODE -t $THREADS $REFERENCE_FASTA $PREFIX.cbreak.patched.fasta > $PREFIX.cbreak.patched.$REFERENCE_NAME.paf 2>> $PREFIX.GPatch.err
